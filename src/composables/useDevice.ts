...
export function useDevice() {
  ...
  const handleCursorMove = async (cursorPoint: CursorPoint) => {
    await handleMouseMove(new PhysicalPosition(cursorPoint))
  ...
  const scheduleCursorMove = () => {
    ...
    requestAnimationFrame(async () => {
      ...
      try {
        await handleCursorMove(cursorPoint)
      } catch (err) {
        error(String(err))
      }

      if (latestCursorPoint !== cursorPoint) {
        scheduleCursorMove()
      }
    })
  }
  ...
  useTauriListen<DeviceEvent>(LISTEN_KEY.DEVICE_CHANGED, ({ payload }) => {
    ...
    if (kind === 'KeyboardPress' || kind === 'KeyboardRelease') {
      if (kind === 'KeyboardPress') {
        generalStore.app.keyboardClickCount += 1
      }
    ...
    switch (kind) {
      case 'MousePress':
        generalStore.app.mouseClickCount += 1
        return handleMouseChange(value)
      case 'MouseRelease':
        return handleMouseChange(value, false)
      case 'MouseMove':
        latestCursorPoint = value
        scheduleCursorMove()
    }
  })
